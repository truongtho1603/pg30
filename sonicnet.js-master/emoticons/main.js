...
function checkSum(number) {
	var sign = 1;
	for (var i = 0; i < 5; i++){
		var b = number.toString(2).charAt(i);
		if (b==1){
			sign = sign * (-1);
		}
	}
...
function encodeMessage(number) {
	var cSum = checksum(number);
	var msg = number + cSum;
	return msg;
}
...
function decodeMessage(message) {
	var b1 = "00011111";
	var b2 = "11100000";
	var number = parseInt(b1,2) & message;
	var chk = parseInt(b2,2) & message;
	var cSum = checkSum(number);
	if (chk != cSum)
